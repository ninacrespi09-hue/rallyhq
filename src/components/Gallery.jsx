...
const TAB_ICON = { Serving: "🏐", Setting: "🎯", Hitting: "💥", Digging: "🤿" };
...
export default function Gallery({ user, media, events }) {
  ...
  const filtered = items.filter((m) => m.category === tab);
        ...
        {MOMENTS.map((m) => {
          ...
          return (
            <button
              key={m}
              onClick={() => setTab(m)}
              className={`flex flex-col items-center gap-0.5 rounded-2xl px-2 py-2.5 text-xs font-semibold ring-1 transition ${
                active
            ...
            >
              <span className="text-lg leading-none">{TAB_ICON[m]}</span>
              <span>{m}</span>
              <span className={active ? "text-white/80" : "text-navy-400"}>{count}</span>
            </button>
          );
        })}
      </div>

      {/* Chronological grid for the active tab (newest first) */}
      <div className="mt-4">
        {filtered.length === 0 ? (
...
function MediaCard({ m, index, onOpen, onLike, onFav }) {
  return (
    <figure
      className="group animate-pop-in mb-3 break-inside-avoid overflow-hidden rounded-2xl bg-white shadow-soft ring-1 ring-navy-100/70"
      style={{ animationDelay: `${Math.min(index, 12) * 40}ms` }}
    >
      <button onClick={() => onOpen(m)} className="relative block w-full overflow-hidden">
        <img
          src={m.url}
        ...
        />
        {m.category && m.category !== "Action" && (
          <span className="absolute left-2 top-2 chip bg-black/45 text-white backdrop-blur-sm">
            {m.category}
          </span>
        )}
        {m.favorite ? <span className="absolute right-2 top-2 text-lg drop-shadow">⭐</span> : null}
      </button>

      <figcaption className="p-3">
        {m.caption && <p className="line-clamp-2 text-sm font-medium text-navy-800">{m.caption}</p>}
        {m.event_title && <p className="mt-0.5 text-xs text-navy-400">{m.event_title}</p>}
        <div className="mt-2 flex items-center gap-1">
          <button
            ...
            className="flex items-center gap-1 rounded-lg px-2 py-1 text-sm transition hover:bg-navy-50 active:scale-90"
          >
            <span>{m.liked ? "❤️" : "🤍"}</span>
          ...
          <button
            ...
            title="Favorite"
            className="grid h-8 w-8 place-items-center rounded-lg text-base transition hover:bg-navy-50 active:scale-90"
          >
            {m.favorite ? "⭐" : "☆"}
          </button>
          <span className="ml-auto truncate text-[11px] text-navy-400">{m.uploader_name || ""}</span>
        </div>
      </figcaption>
    </figure>
  );
      ...
      <div className="relative flex max-h-[95vh] w-full max-w-4xl flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 rounded-2xl bg-white p-4">
          <div className="flex items-center gap-3">
            ...
            {canDelete && (
              <button onClick={onDelete} className="btn bg-red-50 text-red-600 hover:bg-red-100">
                🗑
              </button>
            )}
          </div>
