...
function InsightSections({ insight }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-white/40 p-4 ring-1 ring-blue-200/50">
        <div className="flex items-center gap-2">
          <span className="text-lg">💬</span>
          <h3 className="font-bold text-navy-900">Overview</h3>
          <span
            className={`chip ml-auto ${insight.source === "claude" ? "bg-violet-100 text-violet-700" : "bg-navy-50 text-navy-500"}`}
          >
      ...
      {insight.habitImpact && (
        <div className="rounded-xl bg-sky-50/80 p-4 ring-1 ring-sky-200/60">
...
function Section({ title, items, empty, tone }) {
  ...
  return (
    <div className={`rounded-xl bg-white/40 p-4 ring-1 ${ring}`}>
...
function PlayerVolleyballChat() {
  ...
  return (
    <div className="card overflow-hidden p-0 ring-1 ring-brand-200/50">
      ...
      <form onSubmit={sendMessage} className="flex items-end gap-2 border-t border-blue-200/40 bg-white/30 p-3">
        ...
        />
        <button type="submit" disabled={chatLoading || !input.trim()} className="btn-primary shrink-0 px-4 py-2.5 text-sm">
          Send
        </button>
      </form>
...
export default function AICoachPanel({ role, initialPlayer, initialPlayers }) {
  ...
  async function generateAll() {
    setLoading(true);
    setError("");
    const res = await fetch("/api/ai-coach", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
    const json = await res.json();
    setLoading(false);
  ...
  async function generateOne(playerId) {
    ...
    const json = await res.json();
    setLoadingId(null);
    if (!res.ok) return setError(json.error || "Could not generate.");
    ...
    return (
      <div className="space-y-4">
        ...
        <button onClick={generateAll} disabled={loading} className="btn-primary">
          {loading ? "Analyzing your data…" : playerData?.insight ? "Refresh my AI coach" : "Generate my AI coach"}
        </button>
  ...
  return (
    <div className="space-y-4">
      ...
      <button onClick={generateAll} disabled={loading} className="btn-primary">
        {loading ? "Analyzing all players…" : "Generate all player insights"}
      </button>
