...
import { blockParentApi, isCoach, RSVP_STATUSES } from "@/lib/permissions";
...
/** Set or update a player's RSVP for an event. */
export async function POST(req, { params }) {
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: "Not authenticated." }, { status: 401 });
  if (blockParentApi(user)) {
    return NextResponse.json({ error: "Parents cannot RSVP." }, { status: 403 });
  }
  ...
  if (!assertPlayerOnTeam(targetId, user.team_id)) {
...
/** Clear a player's RSVP (player: self only; coach: any player). */
export async function DELETE(req, { params }) {
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: "Not authenticated." }, { status: 401 });
  if (blockParentApi(user)) {
    return NextResponse.json({ error: "Parents cannot change RSVPs." }, { status: 403 });
  }
