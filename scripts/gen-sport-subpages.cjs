...
const pages = ["schedule/[id]", "players/[id]", "stats/upload", "stats/manual", "schedule/upload"];
...
for (const route of pages) {
  ...
  c = c.replace(/href=\{`\/stats\/manual\?([^`]+)`\}/g, "href={sportPath(sport, `stats/manual?${$1}`)}");

  const out = `src/app/[sport]/${route}/page.js`;
  const dir = out.slice(0, out.lastIndexOf("/"));
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(out, header + c);
